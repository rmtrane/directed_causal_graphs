...
export DATAFILE=../../sim_data/001.txt # ../data/DREAM/insilico_size100_1/0mean/reps.txt
...
export RANDDATAFILE=../../sim_data_permuted/001.txt # data/DREAM/insilico_size100_1/0mean/reps-rand.txt
# Same as DATAFILE, but the files should be the original files where each gene has been randomized over time.


export GENEFILE=../../list_of_genenames.txt # ../data/DREAM/insilico_size100_1/0mean/genes.txt
# List of gene names corresponding to each row of the data files.


export NROWS=`wc -l list_of_genenames.txt | grep -oE "[0-9]+"`
...
export GENES=simulated_data # insilico_size100_1
# Give a name of the dataset.


export NORMALIZATION=not_done #0mean
